...
function SavedPostGrid({ userId }: { userId?: string }) {
  const { profile } = useUser();
  const rqUserId = userId ?? profile.userId;
  const { isLoading, isError, isFetching, isFetchingNextPage, data } = useSavedPosts(rqUserId);
      ...
      <div className="mt-8 px-4 py-8 bg-indigo-100">
        <div className="text-lg  border flex gap-4 flex-col items-center justify-center">
          ...
          <Link href={"/create-post"}>
            <a className="text-base text-indigo-500 hover:underline">Explore posts</a>
          </Link>
        </div>
      </div>
    );
